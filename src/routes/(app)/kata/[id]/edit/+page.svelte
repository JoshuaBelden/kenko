<script lang="ts">
  ...
  let taperUnit = $state(commitment?.type === "taper" ? commitment?.unit ?? "" : "")
  let taperStartDate = $state(commitment?.startDate ? commitment.startDate.slice(0, 10) : new Date().toISOString().slice(0, 10))
  let taperPhases = $state<Array<{ id?: string; weekNumber: number; label: string; dailyLimit: number }>>(
    commitment?.taperPhases?.map((p: any) => ({
      id: p.id,
